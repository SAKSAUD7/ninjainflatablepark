...
import { CalendarDays, Trash2, Plus, Edit } from "lucide-react";
import { format } from "date-fns";
import { toast } from "sonner";

export default function HolidayOpenPage() {
    ...
    async function loadBlocks() {
        try {
            ...
            // Filter for PRIVATE_EVENT or other non-closed types
            const openBlocks = data.filter((block: any) =>
                block.type === "PRIVATE_EVENT" || block.type === "OPEN"
            );
    ...
    return (
        <div className="p-8 space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">Holiday Open Dates</h1>
                    <p className="text-slate-500">Manage special opening hours and private events</p>
                </div>
                <Link
                    href="/admin/booking-blocks/new?type=PRIVATE_EVENT"
                            ...
                            <tr>
                                <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                                    <div className="flex flex-col items-center gap-2">
                                        ...
                                        <p>No special open dates found.</p>
                                    </div>
                                </td>
                            </tr>
                        ) : (
