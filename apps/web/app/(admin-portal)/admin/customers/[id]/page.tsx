...
export default async function CustomerDetailPage({ params }: { params: { id: string } }) {
    ...
    const totalSpent = customer.bookings.reduce((sum, b) => sum + (b.amount || 0), 0);
                ...
                <div className="flex items-start justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-slate-500 text-sm mt-1">Customer ID: #{customer.id.slice(-8).toUpperCase()}</p>
                        </div>
                    </div>
                                ...
                                {customer.bookings.map((booking: any) => (
                                    <tr key={booking.id} className="hover:bg-slate-50 transition-colors">
                                        <td className="px-6 py-4">
                                            <p className="text-sm font-medium text-slate-900">#{booking.id.slice(-6).toUpperCase()}</p>
                                        </td>
                                        <td className="px-6 py-4">
