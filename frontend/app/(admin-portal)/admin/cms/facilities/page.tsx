...
export default function FacilitiesPage() {
    ...
    async function loadData() {
        try {
            const data = await getFacilityItems();
            setItems(data);
        } catch (error) {
    ...
    const handleDelete = async (id: string) => {
        const result = await deleteFacilityItem(id);
        if (result.success) {
    ...
    return (
        <div className="space-y-8">
            <div>
                ...
                <p className="text-slate-500">Manage park facilities and amenities.</p>
            </div>
