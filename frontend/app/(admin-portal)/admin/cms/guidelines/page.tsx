...
export default function GuidelinesPage() {
    ...
    async function loadData() {
        try {
            const data = await getGuidelineCategories();
            setItems(data);
        } catch (error) {
    ...
    const handleDelete = async (id: string) => {
        const result = await deleteGuidelineCategory(id);
        if (result.success) {
    ...
    return (
        <div className="space-y-8">
            <div>
                ...
                <p className="text-slate-500">Manage safety rules and guidelines.</p>
            </div>
